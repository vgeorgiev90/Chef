...
bash 'get_certs' do
  ...
  code <<-EOH
    ...
    scp -i /root/.ssh/k8s root@$IP:/etc/kubernetes/pki/ca.* /etc/kubernetes/pki/.
    scp -i /root/.ssh/k8s root@$IP:/etc/kubernetes/pki/sa.* /etc/kubernetes/pki/.
    scp -i /root/.ssh/k8s root@$IP:/etc/kubernetes/pki/front-proxy-ca.* /etc/kubernetes/pki/.
    scp -i /root/.ssh/k8s root@$IP:/etc/kubernetes/pki/etcd/ca.* /etc/kubernetes/pki/etcd/.
    scp -i /root/.ssh/k8s root@$IP:/etc/kubernetes/admin.conf /root/.kube/config
    EOH
